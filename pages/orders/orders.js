...
Page({
  ...
  getData: function(isclear = 0) {
    ...
    url += "/ziyoutechan/customer/getorders?";
          ...
          var orders = order_list.map(function(order) {
            ...
            if (order.ordertype == 1)
            {
              if (order.order_status == 1 ) {
                ...
                {
                  if (order.givenstatus == 1)
                  {
                    order.order_status_lang = order_status[12];
                  }
                  else
                  {
                    order.order_status_lang = order_status[13];
                  }
            ...
            {
              if (order.finishReason)
              {
                if (order.finishReason == 1)
                {
                  order.order_status_lang = "成团失败"
                }
                else if (order.finishReason == 2)
                {
                  order.order_status_lang = "送礼超时"
                }
                else if (order.finishReason == 3) {
                  order.order_status_lang = "订单取消"
                }
              }
            }

            if (order.orderStatus == 6) {
              if (order.refundProcessStatus) {
                if (order.refundProcessStatus == 0) {
                  order.order_status_lang = "退款订单"
                } 
                else if (order.refundProcessStatus == 1) {
                  order.order_status_lang = "已申请退款"
                }
                else if (order.refundProcessStatus == 2) {
                  order.order_status_lang = "申请已接受"
                }
                else if (order.refundProcessStatus == 3) {
                  order.order_status_lang = "申请被拒绝"
                }
                else if (order.refundProcessStatus == 4) {
                  order.order_status_lang = "卖家回复超时"
                }

                else if (order.refundProcessStatus == 5) {
                  order.order_status_lang = "卖家等待收货"
                }
                else if (order.refundProcessStatus == 6) {
                  order.order_status_lang = "正在退款"
                }
                else if (order.refundProcessStatus == 7) {
                  order.order_status_lang = "已经退款"
                }
                else if (order.refundProcessStatus == 8) {
                  order.order_status_lang = "平台正在协调"
                }
