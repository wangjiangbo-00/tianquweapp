...
Page({
  ...
  getData: function(isclear = 0) {
    ...
    url += "/ziyoutechan/customer/getorderrefunds?";
      ...
      success: function(data) {
        ...
        if (data.returncode == 0) {
          ...
          var orders = order_list.map(function (order) {
            order.refundtime = util.formatTime(order.refundtime);
            ...
            if (order.status == 0)
            {
              ...
              order.statuclass = "status_nomal";
            }
            else if (order.status == 1)
            {
              order.statusstr = "退款成功";
              order.statuclass = "status_success";
            }
            else if (order.status == 2) {
              order.statusstr = "退款失败";
              order.statuclass = "status_fail";
            }
