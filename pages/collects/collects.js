...
Page({
  ...
  getData:function(isclear=0) {
    ...
    if (this.data.all_status == 1)
    {
      this.getGoods(isclear);
    }
    else if(this.data.all_status == 2)
    {
      this.getShops(isclear);
    }
    else if (this.data.all_status == 3)
    {
      this.getArticles(isclear);
    }
                
  },
  getArticles: function (isclear = 0) {
   

      var url = this.baseApiUrl;
    url += "/ziyoutechan/customer/getarticlecollect?";
